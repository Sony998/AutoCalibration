...
df = pd.read_excel(archivo_excel, sheet_name='MONITOR DE SIGNOS VITALES', header=None)
...
while True:
    ...
    saturacion = df.iloc[fila_actual + 40: fila_actual + 44, 1].values.astype(float)
    saturacion_list.append(saturacion)
    fp = df.iloc[fila_actual + 49:fila_actual + 53, 1].values.astype(float)
    fp_list.append(fp)
    fila_actual += 70
...
def agregar_imagenes_pdf2(fondo_path, output_pdf_path, fdepulso_list, diastolica_list, sistolica_list, saturacion_list, fp_list): 
    carta_ancho, carta_alto = letter
    c = canvas.Canvas(output_pdf_path, pagesize=letter)
    c.drawImage(fondo_path, 0, 0, width=carta_ancho, height=carta_alto, preserveAspectRatio=True, mask='auto')
    pdfmetrics.registerFont(TTFont('Arial', 'Formatos/Fuentes/Arial.ttf'))
    ...
    for i in range(3):
        c.drawString(195 + i * 128, 270 , "{:.2f}".format(float(diastolica_list[i])))
    for i in range(3):
...
for certficado in certificados:
    ...
    agregar_imagenes_pdf2(img_fondo_path2, os.path.join(output_directory2, certficado + ".pdf"), fdepulso_list[errorpos], diastolica_list[errorpos], sistolica_list[errorpos], saturacion_list[errorpos], fp_list[errorpos])
