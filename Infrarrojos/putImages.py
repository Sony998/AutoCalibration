...
def main(excel_file):
    ...
    df = pd.read_excel(excel_file, sheet_name=sheetname, header=None)
    ...
    def agregar_imagenes_pdf1(fondo_path, output_pdf_path, nombrecertificado, fecha, nombreEse, metrologo):
        ...
        c.setFont("Arial", 15)
        c.drawString(220, 210, fecha)
        c.drawString(220, 176, fecha)
        c.setFont("Arial", 12)
        c.drawString(220, 148, nombreEse)
        c.setFont("Arial", 15)
        c.drawString(220, 115, metrologo)
        c.save()
    ...
    def agregar_imagenes_pdf2(fondo_path, output_pdf_path, nombrecertificado, incertidumbre, incertidumbre_expandida, primera, segunda, errores_list, temperaturaminima, temperaturamaxima, humedadminima, humedadmaxima, presionbarometrica):
        ...
        pdfmetrics.registerFont(TTFont('ArialI', 'Formatos/Fuentes/ArialI.ttf'))
        c.setFont("ArialI", 14)
        c.drawString(330, 650, str(temperaturaminima))
        c.drawString(405, 650, str(temperaturamaxima))
        c.drawString(355, 610, str(presionbarometrica))
        c.drawString(330, 570, str(humedadminima))
        c.drawString(410, 570, str(humedadmaxima))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generar reportes a partir de un archivo de Excel.")
    parser.add_argument("--f", required=True, help="Ruta del archivo de Excel")
