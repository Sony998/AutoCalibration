...
def generar_grafico_error_lux(archivo_excel):
    ...
    while fila_inicial < len(df):
        ...
        datos_seleccionados = df.iloc[fila_inicial+13:fila_inicial +18, 1].astype(float)  # Fila de los datos seleccionados
...
def generar_grafico_error(archivo_excel):
    ...
    while fila_inicial < len(df):
        ...
        ax.set_ylabel('VALOR OBTENIDO')
        ax.set_title(
            str(nombreEse) + " - " + str(nocertificado) + " - " + "mv / cm3",
            fontsize=10,
            fontweight="bold",
        )   
        output_dir = "OUTPUT/Graficos/mv"
        os.makedirs(output_dir, exist_ok=True)
        plt.savefig(f"{output_dir}/{nocertificado}.png", dpi=300, bbox_inches='tight')
        plt.close(fig)
