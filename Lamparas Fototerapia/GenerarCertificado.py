...
def generar_certificado(archivo_excel, formatometrologo):
    ...
    while True:
        ...
        tipo = sheetname
        inventario = df.iat[fila_inicial + 7, 1] if pd.notna(df.iat[fila_inicial + 3, 1]) else "N.R"
        rango = df.iat[fila_inicial + 4, 1] if pd.notna(df.iat[fila_inicial + 5, 1]) else "N.R"
        print(rango)
