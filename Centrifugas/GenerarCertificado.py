...
def generar_certificado(archivo_excel):
    ...
    while True:
        if fila_inicial >= len(df):
            ...
            tipo = str(df.iat[fila_inicial + 2, 7])
            ...
            fecha = df.iat[5, 14]
            direccion = df.iat[7, 14]
            text_data = {
                (315, 595): ["REVOLUCION POR MINUTO"],
                (315, 575): [tipo],
                (315, 555): [marca],
                (315, 535): [modelo],
                (315, 515): [serie],
                (315, 495): [inventario],
                (315, 475): ["RPM"],
                (315, 455): ["50RPM" if tipo == "CENTRIFUGA" else "Valor Fijo"],
                (315, 435): ["500 - 5000" if tipo == "CENTRIFUGA" else "8500"],
                (315, 390): [nombreEse],
                (315, 370): [direccion],
                (315, 350): [ubicacion],
                (315, 330): [fecha],
