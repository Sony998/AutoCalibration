...
def crear_paginas(archivo_excel):
    ...
    while True:
        ...
        if pd.isna(nota):
            ...
            nota = str(nota)
        datospatron = df.iloc[fila_inicial + 4, 1:11].astype(int).tolist()
        print(datospatron)
        fecha = df.iat[5, 14]
        ...
        patrones_list.append(datospatron)
    ...
    for certficado in certificados:
        ...
        agregar_imagenes_pdf2(img_fondo_path2, os.path.join(output_directory2, certficado + ".pdf"), certficado, incertidumbres[errorpos], incertidumbres_expandidas[errorpos], patrones_list[errorpos], primeras[errorpos], errores_promedio[errorpos], desviaciones[errorpos])
...
def agregar_imagenes_pdf1(fondo_path, output_pdf_path, nombrecertificado, fecha, metrologo, nombreEse):
    ...
    c.drawString(300, 155, fecha)
    c.drawString(300, 125, nombreEse)
    c.drawString(300, 95, metrologo)
    c.save()
def agregar_imagenes_pdf2(fondo_path, output_pdf_path, nombrecertificado, incertidumbre, incertidumbre_expandida, patron, primera, error_promedio, desviacion):
    carta_ancho, carta_alto = letter
    c = canvas.Canvas(output_pdf_path, pagesize=letter)
    c.drawImage(fondo_path, 0, 0, width=carta_ancho, height=carta_alto, preserveAspectRatio=True, mask='auto')
    pdfmetrics.registerFont(TTFont('Arial', 'Formatos/Fuentes/Arial.ttf'))
    ...
    pdfmetrics.registerFont(TTFont('ArialI', 'Formatos/Fuentes/ArialI.ttf'))
    c.setFont("ArialI", 14)
    c.drawString(325, 655, "24")
    c.drawString(438, 655, "27")
    c.drawString(370, 632, "1011")
    c.drawString(325, 600, "52")
    c.drawString(438, 600, "60")
    c.drawString(386, 160, "{:.2f}".format(float(f"{error_promedio:.2f}")))
    c.drawString(386, 135, "{:.2f}".format(float(f"{desviacion:.2f}")))
    c.drawString(386, 110, "{:.2f}".format(float(f"{incertidumbre:.2f}")))
    c.drawString(386, 85, "{:.2f}".format(float(f"{incertidumbre_expandida:.2f}")))
    c.setFont("ArialI", 10)
    for i in range(10):
        c.drawString(140 + i * 42, 278, "{:.2f}".format(float(f"{patron[i]:.2f}")) if isinstance(patron[i], float) else str(patron[i]))
    for i in range(10):
        c.drawString(140 + i * 42, 254, "{:.2f}".format(float(f"{primera[i]:.2f}")) if isinstance(primera[i], float) else str(primera[i]))
    c.save()
